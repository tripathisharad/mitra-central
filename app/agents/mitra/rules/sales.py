...
RULES = {
    "OPEN_SALES_ORDERS": {
        ...
        "sql": (
            "SELECT so.so_nbr AS \"SO Number\", so.so_cust AS \"Customer\", "
    ...
    "SALES_BACKLOG": {
        ...
        "sql": (
            "SELECT sod.sod_nbr AS \"SO Number\", sod.sod_line AS \"Line\", "
            ...
            "FROM sod_det AS sod "
            "JOIN pt_mstr AS pt ON sod.sod_part = pt.pt_part AND sod.sod_domain = pt.pt_domain "
